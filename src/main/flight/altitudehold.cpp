...
{
    return ABS(inclination->values.rollDeciDegrees) < DEGREES_80_IN_DECIDEGREES &&
           ABS(inclination->values.pitchDeciDegrees) < DEGREES_80_IN_DECIDEGREES;
}
...
void correctedWithBaro(float baroAlt, float dt)
{
    ...
    if (ABS(inclination_generalised.values.rollDeciDegrees) > 30 ||
        ABS(inclination_generalised.values.pitchDeciDegrees) > 30) {
        _time_constant_z = 5;
        updateGains();
    } else {
        _time_constant_z = 2;
