...
static const char* const pageTitles[] = { "CLEANFLIGHT", "ARMED", "BATTERY", "SENSORS", "RX", "PROFILE"
#ifdef GPS
        , "GPS"
#endif
...
const pageId_e cyclePageIds[] = { PAGE_PROFILE,
#ifdef GPS
        PAGE_GPS,
#endif
        PAGE_RX, PAGE_BATTERY, PAGE_SENSORS
#ifdef ENABLE_DEBUG_OLED_PAGE
        ,PAGE_DEBUG,
#endif
};
...
void showRxPage(void)
{
    ...
    for (uint8_t channelIndex = 0;
            channelIndex < rxRuntimeConfig.channelCount && channelIndex < RX_CHANNELS_PER_PAGE_COUNT;
            channelIndex += 2) {
...
void showProfilePage(void)
{
    ...
    tfp_sprintf(lineBuffer, "RCE: %d, RCR: %d", controlRateConfig->rcExpo8, controlRateConfig->rcRate8);
    padLineBuffer();
    i2c_OLED_set_line(rowIndex++);
    i2c_OLED_send_string(lineBuffer);

    tfp_sprintf(lineBuffer, "RR:%d PR:%d YR:%d", controlRateConfig->rates[FD_ROLL], controlRateConfig->rates[FD_PITCH], controlRateConfig->rates[FD_YAW]);
    padLineBuffer();
    i2c_OLED_set_line(rowIndex++);
    i2c_OLED_send_string(lineBuffer);
}
...
void showBatteryPage(void)
{
    ...
    if (feature(FEATURE_INA219_VBAT)) {
        tfp_sprintf(lineBuffer, "Volts: %d.%1d Cells: %d", vBatRaw / 10, vBatRaw % 10, batteryCellCount);
        padLineBuffer();
        i2c_OLED_set_line(rowIndex++);
        i2c_OLED_send_string(lineBuffer);
    ...
    if (feature(FEATURE_INA219_CBAT)) {
        // tfp_sprintf(lineBuffer, "Amps: %d.%2d mAh: %d", mAmpRaw / 100, mAmpRaw % 100, mAhDrawn);
        padLineBuffer();
        i2c_OLED_set_line(rowIndex++);
        i2c_OLED_send_string(lineBuffer);
...
void updateDisplay(void)
{
    ...
    if (armedState) {
        ...
        pageState.pageChanging = (pageState.pageFlags & PAGE_STATE_FLAG_FORCE_PAGE_CHANGE) || (((int32_t) (now - pageState.nextPageAt) >= 0L && (pageState.pageFlags & PAGE_STATE_FLAG_CYCLE_ENABLED)));
