...
| `magisv2-rules` | skill | Firmware coding rules and review checklist in one skill, two modes ( writing / reviewing a diff ). `references/invariants.md` ( control-path rules with file:line ), `references/style.md` ( format, banner, units ). |
...
| `run-magisv2 --gate` | skill + driver | `driver.sh --gate <TARGET>` keeps the build log at `Build/<TARGET>/build.log`, runs the warning check and fails the run on new `src/` warnings. Forces a clean build. |
...
| 9 | Exercise the Trail of Bits skills on a known bug | evaluation | All four are installed but none has been run on this tree. Highest value first: `dimensional-analysis` on `flight/` + `sensors/`, then `spec-to-code-compliance` over `fw-architecture-pipeline/`. Try each against a bug whose answer is already known before trusting it. | Open |
...
None of the four knows this tree's invariants — pair them with `magisv2-rules`
